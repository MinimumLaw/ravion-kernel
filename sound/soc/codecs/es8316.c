...
#define DRV_NAME "es8316-component-i2s"
			      ...
			      unsigned int fmt)
{
	...
	iface    = snd_soc_component_read32(codec, ES8316_IFACE);
	adciface = snd_soc_component_read32(codec, ES8316_ADC_IFACE);
	daciface = snd_soc_component_read32(codec, ES8316_DAC_IFACE);
				...
				struct snd_soc_dai *dai)
{
	...
	struct snd_soc_component *codec = snd_soc_rtdcom_lookup(rtd, DRV_NAME);
...
static int es8316_resume(struct snd_soc_component *codec)
{
	...
	ret = snd_soc_component_read32(codec, ES8316_CLKMGR_ADCDIV2_REG05);
	if (!ret) {
...
const static struct snd_soc_component_driver soc_codec_dev_es8316 = {
	.name = DRV_NAME,
	.probe =	es8316_probe,
