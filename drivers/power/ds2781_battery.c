	...
	char *buf, int addr, size_t count, int io)
{
	if (dev_info->mutex_holder == current)
		return w1_ds2781_io_nolock(dev_info->w1_dev, buf, addr, count, io);
	else
	...
	u8 conductance)
{
	...
	ret = ds2781_write(dev_info, &conductance,
				DS2781_RSNSP_REG, sizeof(u8));
	if (ret < 0)
		return ret;

	return ds2781_save_eeprom(dev_info, DS2781_RSNSP_REG);
}
...
static int ds2781_get_rsgain_register(struct ds2781_device_info *dev_info,
	u16 *rsgain)
{
	return ds2781_read16(dev_info, rsgain, DS2781_RSGAIN_MSB_REG);
}
	...
	u16 rsgain)
{
	...
	ret = ds2781_write(dev_info, raw,
				DS2781_RSGAIN_MSB_REG, sizeof(raw));
	if (ret < 0)
		return ret;

	return ds2781_save_eeprom(dev_info, DS2781_RSGAIN_MSB_REG);
}
	...
	int *voltage_uV)
{
	 ...
	 */
	ret = ds2781_read16(dev_info, &voltage_raw,
				DS2781_VOLT_MSB_REG);
	if (ret < 0)
		return ret;
	 ...
	 */
	*voltage_uV = (voltage_raw / 32) * 9760;
	return 0;
}
	...
	int *temperature)
{
	 ...
	 */
	ret = ds2781_read16(dev_info, &temperature_raw,
				DS2781_TEMP_MSB_REG);
	if (ret < 0)
		return ret;
	...
	enum current_types type, int *current_uA)
{
	 ...
	 * the sense resistor.
	 */
	ret = ds2781_read8(dev_info, &sense_res_raw, DS2781_RSNSP_REG);
	if (ret < 0)
		return ret;
	...
	if (type == CURRENT_NOW)
		reg_msb = DS2781_CURRENT_MSB_REG;
	else if (type == CURRENT_AVG)
		reg_msb = DS2781_IAVG_MSB_REG;
	else
		return -EINVAL;
	...
	int *accumulated_current)
{
	 ...
	 * LSB register
	 */
	ret = ds2781_read16(dev_info, &current_raw, DS2781_ACR_MSB_REG);
	if (ret < 0)
		return ret;
	...
	int *capacity)
{
	...
	ret = ds2781_read8(dev_info, &raw, DS2781_RARC_REG);
	if (ret < 0)
		return ret;
	...
	int *charge_now)
{
	 ...
	 * LSB register
	 */
	ret = ds2781_read16(dev_info, &charge_raw, DS2781_RAAC_MSB_REG);
	if (ret < 0)
		return ret;
...
static int ds2781_get_control_register(struct ds2781_device_info *dev_info,
	u8 *control_reg)
{
	return ds2781_read8(dev_info, control_reg, DS2781_CONTROL_REG);
}
	...
	u8 control_reg)
{
	...
	ret = ds2781_write(dev_info, &control_reg,
				DS2781_CONTROL_REG, sizeof(u8));
	if (ret < 0)
		return ret;

	return ds2781_save_eeprom(dev_info, DS2781_CONTROL_REG);
}
	...
	char *buf)
{
	...
	return sprintf(buf, "%d\n",
		 !!(control_reg & DS2781_CONTROL_REG_PMOD));
}
	...
	char *buf)
{
	...
	ret = ds2781_read8(dev_info, &sense_resistor, DS2781_RSNSP_REG);
	if (ret < 0)
		return ret;
	...
	char *buf)
{
	...
	ret = ds2781_read8(dev_info, &sfr, DS2781_SFR_REG);
	if (ret < 0)
		return ret;

	ret = sprintf(buf, "%d\n", sfr & DS2781_SFR_REG_PIOSC);
	return ret;
}
