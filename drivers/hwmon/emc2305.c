...
#define EMC2305_REG_PWM_OUTPUT_CONFIG	0x2b
...
#define EMC2303_DEVICE			0x35
#define EMC2302_DEVICE			0x36
#define EMC2301_DEVICE			0x37
 ...
 * number of edges equal (poles * 2 + 1).
 */
...
static int emc2305_set_pwm(struct device *dev, long val, int channel)
{
	...
	data->cdev_data[channel].cur_state =
		EMC2305_PWM_DUTY2STATE(val, data->max_state, EMC2305_FAN_MAX);
	return 0;
}
...
static int emc2305_set_single_tz(struct device *dev, int idx)
{
	...
	data->cdev_data[cdev_idx].cur_state =
		EMC2305_PWM_DUTY2STATE(data->pwm_min[cdev_idx],
				       data->max_state,
				       EMC2305_FAN_MAX);
	data->cdev_data[cdev_idx].last_state =
		EMC2305_PWM_DUTY2STATE(data->pwm_min[cdev_idx],
				       data->max_state,
				       EMC2305_FAN_MAX);

	/*
	 * Thermal instance target state isn't updated so
	 * make the thermal subsystem update the fan state
	 * if it's used as a cooling device in a thermal zone.
	 */
	data->cdev_data[cdev_idx].cdev->updated = false;
...
static int emc2305_write(struct device *dev,
			 enum hwmon_sensor_types type,
			 u32 attr,
			 int channel,
			 long val)
{
	...
	case hwmon_pwm:
		switch (attr) {
		case hwmon_pwm_input:
			/* If thermal is configured -
			 * handle PWM limit setting. */
			if (IS_REACHABLE(CONFIG_THERMAL)) {
				if (data->pwm_separate)
					...
					cdev_idx = 0;
				data->cdev_data[cdev_idx].last_state =
					EMC2305_PWM_DUTY2STATE(
						val,
						data->max_state,
						EMC2305_FAN_MAX);
				return emc2305_set_cur_state(
					data->cdev_data[cdev_idx].cdev,
					data->cdev_data[cdev_idx].last_state);
			}
...
static int emc2305_read(struct device *dev,
			enum hwmon_sensor_types type,
			u32 attr,
			int channel,
			long *val)
...
static const u32 emc2305_fan_config[] = {
...
static const struct hwmon_channel_info emc2305_fan = {
	.type = hwmon_fan,
	.config = emc2305_fan_config,
};

static const u32 emc2305_pwm_config[] = {
	HWMON_PWM_INPUT,
	HWMON_PWM_INPUT,
	HWMON_PWM_INPUT,
	HWMON_PWM_INPUT,
...
static const struct hwmon_channel_info emc2305_pwm = {
	.type = hwmon_pwm,
	.config = emc2305_pwm_config,
};
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (device < EMC2305_DEVICE || device > EMC2301_DEVICE)
		return -ENODEV;
